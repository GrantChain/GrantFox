...
const MilestonesList = ({ payout }: { payout: Payout }) => {
  ...
  const handleRelease = async (milestoneIdx: number) => {
    await withLoading(payout.payout_id, milestoneIdx, "release", async () => {
      ...
      if (ok) {
        setReleasedAmount(Number(localMilestones[milestoneIdx]?.amount || 0));
                  ...
                  <div className="flex flex-col justify-start items-start gap-2">
                    ...
                    {isReleased && (
                      <Button
                        ...
                        onClick={() => {
                          setReleasedAmount(Number(m.amount || 0));
                          setShowReleaseSuccess(true);
                        }}
                      >
                        See Details
